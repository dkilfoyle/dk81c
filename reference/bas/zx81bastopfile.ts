...
export class Zx81BasToPfile extends EventEmitter {
  ...
  protected createRegex(tokens: string[]): RegExp {
    ...
    const replaced = sorted.map((s) => {
      ...
      if (s === "RND" || s === "PI") {
        ...
        else res = res.replace(/ $/, "[ \t]"); // Don't allow newline
      }
  ...
  public readNumber(): number[] | undefined {
    ...
    if (isNaN(number)) this.throwError(`Number expected but '${numberStr}' found`);
  ...
  protected getZx81Float(value: number): number[] {
    ...
    if (exponent < -129 || exponent > 126) this.throwError("Can't convert to a ZX81 (float) number.");
  ...
  public readQuotedString(): number[] | undefined {
    ...
    do {
      ...
      // Next
      if (this.eos()) this.throwError("Unexpected end of file in string");
    } while (token !== '"');
  ...
  public readNormal(): [number[], boolean] {
    ...
    while (true) {
      ...
      if (!token) this.throwError("Parse error.");
      // Add to buffer
      const tokenNumber = this.normalMapGet(token);
      buffer.push(tokenNumber);

      // Return on 'THEN'
      if (tokenNumber === Zx81Tokens.THEN) break;
    }
  ...
  protected getVariableName(): number[] {
    ...
    if (!result) this.throwError("Variable name expected");
    ...
    if (tokenNumber !== undefined) {
      ...
      this.showWarning(
        "Variable name '" +
          varName +
          "' is same as a BASIC command. This may or may not be a problem. To be on the safe side better rename it.",
        this.lineNr,
        colNr
      );
    }
  ...
  protected readSpecialCode(): number[] | undefined {
    ...
    if (code.startsWith("#")) {
      ...
      if (isNaN(size)) this.throwError("Size expected but got: " + sizeStr);
      if (size >= 0x8000) this.throwError(`Size (${size}) definitely too big`);
      ...
      if (isNaN(number)) this.throwError("Number expected but got: " + code);
      if (number < 0 || number > 255) this.throwError(`Number (${number}) out of range 0..255`);
      buf = [number];
    }
  ...
  protected readLineNumber(): number {
    ...
    if (!digits) this.throwError("Expected a line number");
    // Convert
    const lineNumber = parseInt(digits);
    if (isNaN(lineNumber)) this.throwError("Expected a line number but got '" + digits + "'");
    if (lineNumber < 0 || lineNumber > 9999) this.throwError("Line number out of range [0-9999]: " + lineNumber);
  ...
  protected readInt(min: number, max: number): number {
    const digits = this.readToken(this.regexInt);
    if (!digits) this.throwError("Expected a number");
    // Convert
    const value = parseInt(digits);
    if (isNaN(value)) this.throwError("Expected a number but got '" + digits + "'");
    if (value < min || value > max) this.throwError(`Number out of range [${min}-${max}]: ${value}`);
  ...
  protected skipSpacesAndCont() {
    ...
    if (!match) this.throwError("Space expected ");
    const found = match[0];
    ...
    if (match) {
      ...
      if (cmd) {
        ...
        if (cmd.startsWith("basic-start")) {
          ...
          if (equalSign !== "=") this.throwError("Expected '='");
          this.basicStartLine = this.readLineNumber();
          if (this.basicStartLine <= this.lastLineNumber) this.throwError("Please set the BASIC start line before the definition of the line");
  ...
  protected handleBasicVars() {
    if (this.testReadChar() !== "[") this.throwError("Expected [");
  ...
  protected handleSystemVars() {
    ...
    if (!match) this.throwError("Expected system variable name (or address).");
  ...
  protected readArrayValues(): number[] {
    ...
    while (this.testReadChar() !== "]") {
      ...
      if (value === undefined) this.throwError("Number expected");
      buf.push(value);
  ...
  protected encodeBasic() {
    ...
    while (true) {
      ...
      if (this.parseCommentHeader()) continue;

      // Line number
      const lineNumber = this.readLineNumber();
      if (lineNumber <= this.lastLineNumber) this.throwError("Line number (" + lineNumber + ") must be bigger than previous line.");
      ...
      while (true) {
        ...
        if (!token) this.throwError("Unknown command");
    ...
    if (this.basicStartLine !== -1) this.throwError("Start line not found: " + this.basicStartLine);
  }
  ...
  public createPfile(): number[] {
    ...
    const pFile: number[] = [...sysVarsValues, ...this.basicCodeOut, ...dfileData, ...variables];
  ...
  protected throwError(message: string, lineNr = this.lineNr, colNr = this.colNr): never {
    ...
    if (k >= 0) curText = curText.substring(0, k);
    else curText += "...";
    let msg = message;
    if (k > 0) msg += ": '" + curText + "'";
