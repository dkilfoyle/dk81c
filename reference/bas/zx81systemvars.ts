...
export class Zx81SystemVars {
  ...
  public setValuesFromArray(values: Uint8Array) {
    if (values.length !== 16509 - 16393) throw Error("Invalid system variable values array.");
  ...
  protected getSysVarInfo(name: string): { address: number; size: number } {
    ...
    if (!sysVarInfo) {
      ...
      if (isNaN(address)) throw Error(`Invalid system variable name: ${name}.`);
      const addrName = this.sysVarsAddresses.get(address);
      if (addrName === undefined) throw Error(`Invalid system variable address: ${name}.`);
  ...
  public setSysVarAtAddr(addr: number | string, value: number | number[]) {
    ...
    if (typeof addr === "number") {
      ...
      if (addrName === undefined) throw Error("Invalid system variable address.");
    ...
    if (typeof value === "number") {
      ...
      if (size === 1 && highValue !== 0) throw Error(`Invalid value: ${value}. Expected a byte but got a word.`);
      value = [value & 0xff];
      if (size > 1) value.push(highValue);
    }
    if (value.length !== size) throw Error(`Invalid value: ${value}. Expected ${size} byte(s) but got ${value.length}.`);
    const minSize = Math.min(value.length, size);
  ...
  protected getSysVarAtAddr(addr: number | string): number {
    ...
    if (typeof addr === "number") {
      const addrName = this.sysVarsAddresses.get(addr);
      if (addrName === undefined) throw Error(`Invalid system variable address: ${addr}.`);
      addr = addrName as string;
    }
    // Get info for sys variable
    const sysVarInfo = this.getSysVarInfo(addr);
    ...
    if (size > 1) word += 256 * this.sysVarsValues[index + 1];
    return word;
  }
