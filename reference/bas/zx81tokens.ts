...
export class Zx81Tokens {
  ...
  public static convertBasLine(buffer: Uint8Array, bracketized = false): string {
    ...
    for (let i = 0; i < length; i++) {
      ...
      // Number?
      if (!rem && !quoted && token === Zx81Tokens.NUMBER) {
        // Number (is hidden)
    ...
    if (rem && token === Zx81Tokens.SPACE) txt = txt.slice(0, -1) + "[0]";
  ...
  protected static convertBufToZx81Float(buf: Uint8Array): number {
    if (buf.length !== 5) throw Error("Expected 5 bytes for a ZX81 float number.");
    const mantissa = (buf[1] << 24) + (buf[2] << 16) + (buf[3] << 8) + buf[4];
    if (mantissa === 0 && buf[0] === 0) return 0;
  ...
  protected static getLastNumber(txt: string): string {
    ...
    while (k >= 0) {
      ...
      if ((c >= "0" && c <= "9") || c === ".") {
        ...
        if (expFound > 0) break; // Exponent already found
        expFound++;
        found = c + found;
      } else if (c === "+" || c === "-") {
        if (expFound > 0) break; // Other than exponent, no +/- allowed
        // A 'E' is expected before the sign.
        k--;
        if (k < 0) break; // String ended
        if (txt[k] !== "E") break; // No 'E' so strings ends without sign.
        expFound++;
        found = "E" + c + found;
      } else break;
      // Next
