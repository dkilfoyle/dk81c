...
import { zxbasicClientConfig } from "@/basic/monaco/zxbasic-client-config";
...
const sourceCodes = import.meta.glob("../basic/examples/**/*.bas", { as: "raw" });

export const BasicEditing = ({ zxworker }: { zxworker: React.RefObject<Worker | null> }) => {
  const basicWrapperRef = useRef<LanguageClientWrapper | null>(null);
  const editorAppRef = useRef<EditorApp | null>(null);
  const [source, setSource] = useState("scratch.bas");

  const clickLoad = useCallback(() => {
    if (basicWrapperRef.current) {
      basicWrapperRef.current.getLanguageClient()?.sendNotification("client/pfile_req", { filename: "workspace/source.bas" });
    }
  }, [basicWrapperRef]);

  const onLanguageClientsLoad = useCallback(
    (lcsManager?: LanguageClientsManager) => {
      const wrapper = lcsManager?.getLanguageClientWrapper("zxbasic");
      if (!wrapper) throw Error("zxbasic !wrapper");
      basicWrapperRef.current = wrapper;
      const lc = wrapper.getLanguageClient();
      if (!lc) throw Error("zxbasic !lc");
  ...
  const files = useMemo(() => {
    return createListCollection({
      items: Object.keys(sourceCodes).map((x) => ({ label: x.replace("../basic/examples/", ""), value: x })),
    });
  }, []);
  ...
  return (
    <Flex direction={"column"} height="100%" gap={4}>
      ...
      <MonacoEditorReactComp
        ...
        editorAppConfig={zxbasicClientConfig.editorAppConfig}
        languageClientConfig={zxbasicClientConfig.languageClientConfig}
